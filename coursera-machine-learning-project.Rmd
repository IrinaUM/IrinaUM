...
We train three baseline machine learning algorithms: Gradient Boosted Trees, Random Forest and Decision Trees. Instruct train to use 4-fold CV to select optimal tuning parameters. 4-fold CV because in the scientific article 4-fold CV shows the best result.
...
Last step is the prediction a target variable "classe" for each of the 20 observations from validation data ("pml-testing.csv" file). We use best model for prediction. Before prediction we clean validation data.
...
table_pred <- table(answers, pred)
table_pred
```

So here we obtain final answers and can see that we have only one error in prediction of "A" classe, that give as 95% of prediction on validation data with Random Forests model.
